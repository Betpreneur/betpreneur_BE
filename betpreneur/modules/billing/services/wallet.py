...
from betpreneur.modules.billing.services.delivery import Delivery, resolve_delivery
...
class TokenWalletService:
    ...
    def _delivery_verdict(self, reservation: TokenReservation):
        """Ask the module that owns this reference whether it delivered.

        Expiry means "we never delivered, so give the tokens back". That is only
        true if the work did not complete. A reservation can still be sitting in
        `reserved` after successful work -- consume_reservation is best-effort at
        the call site, so a DB blip there leaves the escrow open. Refunding it
        would hand back tokens for something the user already received.
        """
    ...
    def expire_stale_reservations(self, *, now=None, limit: int | None = None) -> dict[str, Any]:
        ...
        for reservation_id in reservation_ids:
            try:
                ...
                verdict = self._delivery_verdict(reservation)
                delivery = verdict.status
                if delivery == Delivery.IN_FLIGHT:
                    deferred += 1
                    continue
                if delivery == Delivery.DELIVERED:
                    ...
                    billable_tokens = (
                        verdict.billable_tokens
                        if verdict.billable_tokens is not None
                        else int(reservation.amount or 0)
                    )
