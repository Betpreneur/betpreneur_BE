...
from ..domain.leg_state import may_publish_probability
from .calibration_source import settled_legs
...
class TicketRiskService:
    def calibration(self) -> Calibration:
        ...
        for leg in settled_legs():
            band = bands[_band_for(float(leg.advisory_score))]
            band["settled"] += 1
            total += 1
            if leg.won:
                band["wins"] += 1
...
def _correlation_for(items: list[dict[str, Any]], legs: list[LegRisk]):
    ...
    """
    from betpreneur.modules.scoring.api import CorrelationResult, combine, score_model_service
...
def _tier_counts(legs: list[LegRisk]) -> dict[str, int]:
    counts = dict.fromkeys(TIER_LABELS, 0)
    for leg in legs:
...
def _leg_score(item: dict[str, Any]) -> float | None:
    ...
    return float_or_none(item.get("advisory_score") or market.get("advisory_score"))


def _repair_score(item: dict[str, Any]) -> float | None:
    replacement = item.get("replacement_market") or item.get("recommended_market") or {}
    return float_or_none(replacement.get("advisory_score"))
...
def _has_scored_quantitative_advisory(item: dict[str, Any]) -> bool:
    from betpreneur.modules.markets.api import QUANTITATIVE, assessment_type_for
...
def _data_confidence_percent(item) -> float | None:
    ...
    cap = float_or_none(_capability(item).get("confidence_cap"))
