...
from betpreneur.modules.catalog.api import ProviderPlayerMap
...
from betpreneur.modules.scoring.api import RESULT_DEPENDENT_FAMILIES, build_score_matrix
...
class StatPalMarketAdvisoryService:
        ...
        *,
        ...
        if spec.engine in {SCORE_MATRIX_ENGINE, COUNT_MODEL_ENGINE}:
            if spec.engine == SCORE_MATRIX_ENGINE:
                from betpreneur.modules.scoring.api import score_matrix_evaluator as engine
            else:
                from betpreneur.modules.scoring.api import count_market_evaluator as engine
    ...
    def _player_availability_block(self, descriptor: MarketDescriptor, fixture):
        ...
        """
        from betpreneur.modules.scoring.api import player_availability_service

        subject = str(descriptor.subject or descriptor.raw or "")
        if not subject:
            return None

        from betpreneur.modules.scoring.api import lineup_service
    ...
    def _evaluate_score_matrix_market(
        ...
        from betpreneur.modules.scoring.api import outcome_probability

        matrix = build_score_matrix(home_expected, away_expected)
        probability, push = outcome_probability(descriptor, matrix)
        if probability is None:
            return StatPalAdvisory(
                available=False,
                score=None,
