"""Admin for runs, picks and backing."""
...
from betpreneur.modules.explanations.api import council_review
...
from betpreneur.modules.picks.services.performance import performance_dashboard
from betpreneur.modules.picks.tasks import generate_daily_picks, recover_daily_run
from betpreneur.modules.pricing.api import assess_recommendation

# Settlement and analytics both sit above picks, so neither may be imported
# here. Dispatching by name keeps the layer order intact — Celery resolves the
# task on the worker.
...
def queue_result_settlement(modeladmin, request, queryset):
    ...
    for target_date in queryset.values_list("target_date", flat=True).distinct():
        task = current_app.send_task(SETTLE_PICKS_TASK, args=[target_date.isoformat()])
        task_ids.append(task.id)
    messages.success(
        request,
...
def queue_auditor(modeladmin, request, queryset):
    task_ids = []
    for algo_run in queryset:
        task = current_app.send_task(AUDITOR_TASK, args=[None, algo_run.target_date.isoformat()])
        task_ids.append(task.id)
    messages.success(
        request,
...
class AlgoRunAdmin(admin.ModelAdmin):
    ...
    def _prediction_top_rank(self, prediction):
        from .views import market_display_score
...
class PickAdmin(admin.ModelAdmin):
    ...
    def queue_settlement_for_pick_dates(self, request, queryset):
        ...
        for match_date in dates:
            task = current_app.send_task(SETTLE_PICKS_TASK, args=[match_date.isoformat()])
            task_ids.append(task.id)
        messages.success(
            request,
            f"Queued {len(task_ids)} settlement task(s): {', '.join(task_ids)}",
