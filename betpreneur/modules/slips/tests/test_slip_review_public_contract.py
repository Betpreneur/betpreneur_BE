...
from betpreneur.modules.billing.api import TokenTransaction, TokenWallet
from betpreneur.modules.markets.api import describe_market
from betpreneur.modules.pricing.api import SCORE_BANDS, Calibration
...
class SlipReviewPublicContractTests(SimpleTestCase):
    ...
    def test_slip_review_debug_logging_exposes_decision_inputs(self):
        from betpreneur.modules.slips.interface.views import _log_slip_review_debug

        summary = _manual_review_summary([_sample_replace_result()])
        review = type("Review", (), {"id": 33, "status": "completed", "source": "sportybet"})()

        with self.assertLogs("betpreneur.modules.slips.interface.views", level="INFO") as captured:
