...
from betpreneur.modules.pricing.api import statpal_market_advisory as advisory
...
class ThesisSideTests(SimpleTestCase):
    def test_each_result_market_reports_the_team_it_backs(self):
        for name, side in (
            ...
            with self.subTest(market=name):
                self.assertEqual(result_thesis_side(_m(name)), side)
    ...
    def test_a_market_backing_both_sides_has_no_direction(self):
        ...
        self.assertEqual(result_thesis_side(_m("DC: 12")), "")
...
class ThesisPreservationTests(SimpleTestCase):
    def test_a_double_chance_is_not_flipped_to_the_other_side(self):
        ...
        self.assertFalse(result_replacement_preserves_user_thesis(_m("DC: 1X"), _m("DC: X2")))
        self.assertFalse(result_replacement_preserves_user_thesis(_m("DC: X2"), _m("DC: 1X")))

    def test_a_draw_no_bet_is_not_flipped_either(self):
        self.assertFalse(result_replacement_preserves_user_thesis(_m("DNB Home"), _m("DC: X2")))

    def test_a_double_chance_may_move_to_a_market_on_the_same_side(self):
        self.assertTrue(result_replacement_preserves_user_thesis(_m("DC: 1X"), _m("DNB Home")))

    def test_match_result_behaviour_is_unchanged(self):
        self.assertTrue(result_replacement_preserves_user_thesis(_m("Home Win"), _m("DC: 1X")))
        self.assertTrue(result_replacement_preserves_user_thesis(_m("Home Win"), _m("AH Home +0.5")))
        self.assertFalse(result_replacement_preserves_user_thesis(_m("Home Win"), _m("DC: X2")))
        self.assertFalse(result_replacement_preserves_user_thesis(_m("Home Win"), _m("DC: 12")))

    def test_a_draw_pick_only_moves_to_another_draw(self):
        self.assertFalse(result_replacement_preserves_user_thesis(_m("Draw"), _m("DC: 1X")))

    def test_a_directionless_pick_only_keeps_its_own_market(self):
        self.assertFalse(result_replacement_preserves_user_thesis(_m("DC: 12"), _m("DC: 1X")))
