...
* `allows_broad_replacement` returned `group not in {"unknown"}` -- true for everything.
...
from betpreneur.modules.slips.domain.slip_analysis import _market_is_better_for_slip
from betpreneur.modules.slips.services.slip_presentation import _replacement_market_for_slip
...
class PlayerMarketIdentityTests(SimpleTestCase):
    ...
    def test_a_player_shots_on_target_pick_groups_with_player_markets(self):
        ...
        self.assertEqual(market_family_group(market), "player")
...
class SpecialistGuardTests(SimpleTestCase):
    ...
    def test_specialist_groups_refuse_a_cross_family_swap(self):
        for name, family in (
            ...
            with self.subTest(market=name):
                self.assertFalse(allows_broad_replacement(self._market(name, family)))

    def test_a_goals_pick_may_still_take_a_broad_alternative(self):
        self.assertTrue(allows_broad_replacement(self._market("Over 1.5", "total_goals")))
