# ================================================================
# GRIND ALGO — Cloud Run Runner v8  (Dual-API Architecture)
# ================================================================
#
# DATA SOURCES:
#   football-data.org  — Big 5 + continental (PL, La Liga, CL etc.)
#                        Form fetched per team. Free, unlimited.
#   API-Football       — Fills the gaps (UECL, MLS, Brazil, 2.Bund,
#                        Segunda, Friendlies, leagues FD doesn't cover)
#                        Budget: 100 req/day (1 global net + predictions)
#   The Odds API       — Real bookmaker odds for EV blending
#
# REQUEST BUDGET (API-Football):
#   Step 1 — Global Net:  1 request
#   Step 4 — Predictions: 1 per API-only game (not FD games)
#   Total:  ~35-55/day on a busy Saturday -> well within 100/day limit
#
# FLOW:
#   1. FD.org   -> fetch all games for target date across all comps
#   2. API-Ftbl -> global net (1 req) -> filter VIP leagues -> deduplicate
#   3. Score FD games using FD team form (free, no API budget)
#   4. Score API-only games using /predictions endpoint (6.5s sleep)
#   5. Merge all -> 19-param EV engine -> picks -> PDF -> Drive
# ================================================================
...
_KEY_DEFAULT    = "/secrets/grind_key.json" if os.path.exists("/secrets/grind_key.json") else "/app/grind_key.json"
...
# ── FD COMPETITIONS (all free, unlimited) ────────────────────────
FD_COMPS = [
    "PL","ELC","PD","SD","BL1","BL2","SA","FL1",
    "DED","PPL","CL","EL","BSA","MLS",
]

# ── API-FOOTBALL VIP LEAGUES (gaps FD doesn't cover well) ────────
APS_VIP_LEAGUES = {
...
# ── PHASE 1: FD.ORG FIXTURE FETCHER ──────────────────────────────
def fetch_fd_fixtures(target_date):
    headers  = {"X-Auth-Token": FD_TOKEN}
    fixtures = []
    seen     = set()
    for comp in FD_COMPS:
        try:
            r = requests.get(f"{FD_BASE}/competitions/{comp}/matches",
                             headers=headers,
            ...
            for m in r.json().get("matches",[]):
                hname = m["homeTeam"].get("shortName") or m["homeTeam"].get("name","")
                aname = m["awayTeam"].get("shortName") or m["awayTeam"].get("name","")
            ...
            log.warning(f"FD {comp}: {e}")
    log.info(f"FD: {len(fixtures)} fixtures")
    return fixtures

# ── PHASE 2: API-FOOTBALL GLOBAL NET ─────────────────────────────
def fetch_aps_global_net(target_date):
    headers = {"x-apisports-key": APS_KEY}
    try:
        log.info("APS global net (1 req)...")
        r = requests.get(f"{APS_BASE}/fixtures", headers=headers,
                         params={"date":target_date,"timezone":"Africa/Lagos"},
                         timeout=20)
        time.sleep(6.5)
        if r.status_code == 200:
            log.info(f"APS: {r.json().get('results',0)} global fixtures")
            return r.json().get("response",[])
        log.error(f"APS global net: {r.status_code}")
        return []
    except Exception as e:
        log.error(f"APS global net error: {e}")
        return []

def filter_aps_fixtures(aps_all, fd_fixtures, target_date):
    fd_keys = {normalize(f"{fx['hname']}{fx['aname']}") for fx in fd_fixtures}
    api_only = []
    for f in aps_all:
        league_id = f.get("league",{}).get("id")
        status    = f.get("fixture",{}).get("status",{}).get("short","")
        if status in ("FT","AET","PEN","CANC","ABD"): continue
        if league_id not in APS_VIP_LEAGUES: continue
        hname = f["teams"]["home"]["name"]
        aname = f["teams"]["away"]["name"]
        key   = normalize(f"{hname}{aname}")
        if key in fd_keys:
            for fx in fd_fixtures:
                if fuzzy(fx["hname"],hname) and fuzzy(fx["aname"],aname):
                    fx["aps_id"] = f["fixture"]["id"]
                    break
            continue
        api_only.append({
            "fixture":  f"{hname} vs {aname}",
    ...
    log.info(f"APS-only unique games: {len(api_only)}")
    return api_only
...
def fetch_prediction_data(fixture_id):
    headers = {"x-apisports-key": APS_KEY}
    try:
        r = requests.get(f"{APS_BASE}/predictions", headers=headers,
...
def get_fd_form(team_id, before_date, team_name=None):
    cache_key = f"fd_{team_id}_{before_date}"
    if cache_key in _form_cache: return _form_cache[cache_key]
    headers = {"X-Auth-Token": FD_TOKEN}
    resolved = team_id
    if (not team_id or team_id==0) and team_name:
        try:
            r = requests.get(f"{FD_BASE}/teams", headers=headers,
                             params={"name":team_name}, timeout=10)
            time.sleep(7)
            if r.status_code==200:
                teams = r.json().get("teams",[])
                if teams: resolved = teams[0].get("id")
    ...
    for attempt in range(2):
        try:
            ...
            date_from = (target_dt - timedelta(days=365)).strftime("%Y-%m-%d")
            time.sleep(7)
            r = requests.get(f"{FD_BASE}/teams/{resolved}/matches",
                             headers=headers,
                             params={"status":"FINISHED","limit":12,
                                     "dateTo":before_date,"dateFrom":date_from},
                             timeout=15)
            if r.status_code==429:
                log.warning("FD 429 — waiting 65s")
            ...
            wins=goals=conceded=btts=o25=cs=games=0; results=[]
            for m in matches[-8:]:
                try:
                    hid = m.get("homeTeam",{}).get("id",0)
                    hg  = m["score"]["fullTime"].get("home") or 0
                    ag  = m["score"]["fullTime"].get("away") or 0
                    s,c = (hg,ag) if hid==resolved else (ag,hg)
                    r_  = "W" if s>c else "D" if s==c else "L"
                    wins+=(r_=="W"); goals+=s; conceded+=c
                    btts+=(s>0 and c>0); o25+=(s+c>2); cs+=(c==0)
                    games+=1; results.append(r_)
                except Exception: continue
            if not games: break
            streak=0
            for r_ in results[::-1]:
                if r_==results[-1]: streak+=1
                else: break
            if results[-1]=="L": streak=-streak
            result = {"wins":wins,"avg_scored":round(goals/games,2),
                      "avg_conceded":round(conceded/games,2),
                      "btts_count":btts,"over25_count":o25,
                      "clean_sheets":cs,"games":games,"streak":streak,
                      "attack_str":0.5,"defence_str":0.5}
            _form_cache[cache_key] = result
            return result
        except Exception as e:
            log.warning(f"FD form {resolved}: {e}")
            if attempt==1: break
    result = _default_form()
    _form_cache[cache_key] = result
    return result
...
ODDS_SPORT_MAP = {
    ...
    "DED":"soccer_netherlands_eredivisie","BSA":"soccer_brazil_campeonato",
    "MLS":"soccer_usa_mls","ELC":"soccer_efl_champ","PPL":"soccer_portugal_primeira_liga",
    "BL2":"soccer_germany_bundesliga2","SD":"soccer_spain_segunda_division",
}

def get_real_odds(comp_code, hname, aname):
    sport = ODDS_SPORT_MAP.get(comp_code)
    if not sport: return {}
    if not ODDS_KEY or ODDS_KEY=="YOUR_KEY_HERE": return {}
    if sport not in _odds_cache:
        try:
            r = requests.get(f"{ODDS_BASE}/sports/{sport}/odds",
                             ...
                             timeout=12)
            time.sleep(0.3)
            _odds_cache[sport] = r.json() if r.status_code==200 else []
        except Exception:
            _odds_cache[sport] = []
    for game in _odds_cache.get(sport,[]):
        sh = normalize(game.get("home_team",""))
        sa = normalize(game.get("away_team",""))
        if fuzzy(hname,sh) and fuzzy(aname,sa):
            od={}
            for bm in game.get("bookmakers",[]):
                for mkt in bm.get("markets",[]):
                    ...
                    for o in mkt.get("outcomes",[]):
                        ...
                        if mk=="h2h":
                            if n==game["home_team"]: od["hw"]=p
                            elif n==game["away_team"]: od["aw"]=p
                            elif n=="Draw": od["d"]=p
                        elif mk=="totals":
                            pt=o.get("point",0)
                            if pt==2.5: od["o25" if n=="Over" else "u25"]=p
                            elif pt==1.5: od["o15" if n=="Over" else "u15"]=p
                if "hw" in od and "aw" in od: return od
    return {}
...
ODDS_KEYS_MAP = {"Home Win":"hw","Away Win":"aw","Over 1.5":"o15",
...
def generate_and_upload_pdf(drive, bankers, value_gems, wild_cards,
    ...
    story.append(Paragraph("Daily Betting Intelligence Report  |  v8  |  Dual-API Architecture", sSubT))
    story.append(Paragraph(
        f"Date: <b>{target_date}</b>   |   "
        f"Generated: <b>{now_wat.strftime('%d %B %Y  %H:%M WAT')}</b>   |   "
        f"Bankroll: <b>N{bankroll:,.0f}</b>   |   "
        f"Data: FD + API-Football + The Odds API",
        sMeta))
    ...
    scorecard_expl = (
        ...
        "<b>APS</b> = API-Football (wider global coverage via predictions endpoint). "
    ...
    meth_txt = (
        ...
        "<b>Phase 2 — Global Net (API-Football):</b> A single API-Football request fetches all "
        ...
        "API-Football, the /predictions endpoint is called (1 request per fixture). The returned "
        ...
        "<b>Odds Blending:</b> Where real bookmaker odds are available (via The Odds API), they "
    ...
    glossary = [
        ...
        ["APS", "Data from API-Football — wider global coverage via /predictions endpoint."],
    ...
    story.append(Spacer(1, 2*mm))
    story.append(Paragraph(
        f"THE GRIND ALGO v8  |  {target_date}  |  Dual-API (FD + APS + OddsAPI)  |  "
...
# ── MAIN RUNNER ───────────────────────────────────────────────────
def run_daily_algo():
    log.info("=== GrindAlgo v8 Dual-API Run ===")
    ...
    # Phase 1: FD fixtures (free)
    fd_fixtures = fetch_fd_fixtures(target_date)

    # Phase 2: API-Football global net (1 req) + filter
    aps_all      = fetch_aps_global_net(target_date)
    aps_only_fxs = filter_aps_fixtures(aps_all, fd_fixtures, target_date)

    # Cap API-only games to protect 100 req/day budget
    MAX_APS = 89   # 1 used for global net = 99 left; 89 predictions + 9 buffer
    if len(aps_only_fxs) > MAX_APS:
        log.warning(f"Capping APS games: {len(aps_only_fxs)} -> {MAX_APS}")
        aps_only_fxs = aps_only_fxs[:MAX_APS]

    total = len(fd_fixtures)+len(aps_only_fxs)
    log.info(f"Total games: {total} ({len(fd_fixtures)} FD + {len(aps_only_fxs)} APS-only)")
    ...
    # Phase 3: Score FD games with FD form (no APS budget)
    log.info(f"Scoring {len(fd_fixtures)} FD games...")
    for idx,fx in enumerate(fd_fixtures):
        try:
            log.info(f"  [FD {idx+1}/{len(fd_fixtures)}] {fx['fixture']}")
            hf = get_fd_form(fx["hid"],target_date,team_name=fx["hname"])
            af = get_fd_form(fx["aid"],target_date,team_name=fx["aname"])
            h2h = {"games":5,"t1w":2,"o25":3,"btts":2}
            real_odds = get_real_odds(fx["code"],fx["hname"],fx["aname"])
            confs = score_fixture(hf,af,h2h,real_odds,api_preds=None)
            all_confs.append(confs); scored_fxs.append(fx); odds_list.append(real_odds)
            log.info("    FD scored OK")
        except Exception as e:
            log.warning(f"FD score error {fx['fixture']}: {e}")
    ...
    # Phase 4: Score APS-only games with /predictions (budget: 1 req each)
    log.info(f"Scoring {len(aps_only_fxs)} APS-only games with /predictions...")
    for idx,fx in enumerate(aps_only_fxs):
        try:
            log.info(f"  [APS {idx+1}/{len(aps_only_fxs)}] {fx['fixture']}")
            ...
            confs = score_fixture(_default_form(),_default_form(),{"games":5,"t1w":2,"o25":3,"btts":2},{})
            all_confs.append(confs); scored_fxs.append(fx); odds_list.append({})
        time.sleep(6.5)   # APS rate limit: 10 req/min
    ...
    result = {"status":"success","date":target_date,
              "fd_fixtures":len(fd_fixtures),"aps_fixtures":len(aps_only_fxs),
